                                    ...
                                    <TableBody>
                                        {auctionList.map((row) => (
                                            <TableRow key={row._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                                ...
                                                <TableCell>
                                                    <Chip label={row.status == AuctionStatusEnum.ACTIVE ? "Активний" : "Завершений"}
                                                          color={row.status == AuctionStatusEnum.ACTIVE ? "success" : "primary"}
                                                          variant="outlined" />
                                                </TableCell>
                                                <TableCell>
                                                    <ButtonGroup variant="contained" aria-label="Basic button group">
                                                        <Button onClick={() => { navigate(`/dashboard/auction/${row._id}/edit`) }}>
                                                            <EditIcon />
                                                        </Button>
                        ...
                        <Grid container spacing={2}>
                            {auctionList.map((row) => (
                                <Grid item xs={12} key={row._id}>
                                    <Card>
                                        {row.thumbnail && (
                                            <img style={{ height: 100, width: '100%', objectFit: 'cover' }} src={`${row.thumbnail}`} />
                                        )}
                                        <CardContent>
                                        ...
                                        <CardActions>
                                            <Button size="small" onClick={() => navigate(`/dashboard/auction/${row._id}/edit`)}>
                                                <EditIcon />
                                                Переглянути
                                            </Button>
